...
export class AttendanceComponent implements OnInit,OnDestroy {
 ...
 async view(row:any){
  ...
  if(!this.detailDialog.nativeElement.open)this.detailDialog.nativeElement.showModal()
  if(!row.id)return
  try{this.detail.set(await firstValueFrom(this.http.get<any>(this.api+'/'+row.id)))}catch(e:any){this.dialogError=e.error?.message||e.message}finally{this.detailLoading=false}
 }
