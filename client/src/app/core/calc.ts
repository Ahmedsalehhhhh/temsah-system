export const clampNum = (v: any): number => {
  const n = Number(v)
  return Number.isFinite(n) ? n : 0
}

export interface StreamerRules {
  defaultRate: number
...
export function calcStreamer(
  perf: { score?: number; days?: number; hours?: number; bonus?: number; deduction?: number } | undefined,
  profile: { rate?: number } | undefined,
  rules: StreamerRules,
  ...
  return { score, days, hours, scorePct, scoreAmt, daysPct, daysAmt, extraAmt, extraPct, rate, totalPct, totalScore, cash, bonus, deduction, total, egp }
}

export function calcRecruiter(
  records: { tier: number }[],
  tierAmounts: TierAmounts,
  adjustment: { bonus?: number; deduction?: number } | undefined
) {
  ...
  const amount =
    counts[1] * tierAmounts[1] + counts[2] * tierAmounts[2] + counts[3] * tierAmounts[3] +
    counts[4] * tierAmounts[4] + counts[5] * tierAmounts[5]
...
export function calcStaff(row: { bonus?: number; deduction?: number } | undefined, baseSalary: number, tierBonus = 0) {
  const bonus = clampNum(row?.bonus)
  const deduction = clampNum(row?.deduction)
  return { baseSalary: clampNum(baseSalary), recruiterBonus: clampNum(tierBonus), bonus, deduction, total: clampNum(baseSalary) + clampNum(tierBonus) + bonus - deduction }
}
